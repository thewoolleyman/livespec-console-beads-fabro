...
file, line, mutation-operator)` identity -- and persist it on the filed
work-item. It MUST NOT file a chore when an open (non-closed) chore
already carries that signature, filing only when none does. The
pre-existing open chore MAY be stale -- its detail can lag, and it does
not re-surface a new finding masked behind it -- which is acceptable
because the soak is expensive and any fix re-runs the soak manually to
surface the current finding set; the obligation is that each distinct
finding is tracked at least once, not that every open chore stays
current.
