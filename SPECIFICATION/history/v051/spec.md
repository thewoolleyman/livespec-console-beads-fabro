...
The Dispatcher's routine dispositions are governed by the orchestrator-owned
`dispatcher.*` policy settings; at the time of writing that set included
`auto_approve_ready`, `merge_on_review_cap`, `acceptance_mode`,
`review_fix_cap`, `acceptance_rework_cap`, and `wip_cap`, and it GROWS as the
orchestrator ratifies further keys, so that enumeration is illustrative and
MUST NOT be read as a fixed or exhaustive set (the owning declaration: repo
...
A dispatcher setting admits a **per-item override** unless the orchestrator
declares otherwise, and the console lets the operator set that override
against a single work-item. `wip_cap` is a standing exception: it is a
per-repo concurrency ceiling, so a per-item value is structurally meaningless,
and the console MUST NOT offer a per-item override for it. The console MUST
take which settings admit an override from the orchestrator's published
declaration rather than from a count fixed here. Where
