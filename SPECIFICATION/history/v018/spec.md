...
return every decidable item to human routing. The single persistent
permission is the orchestrator plane's `dispatcher.autonomous_mode` key; the
console does not persist its own preference -- it derives and reflects that
key, sets it through the orchestrator's published command surface, and audits
each change, per `contracts.md` -> Autonomous Mode. Its operator-observable
safety constraints live in `constraints.md` -> Autonomous-Mode Safety.
