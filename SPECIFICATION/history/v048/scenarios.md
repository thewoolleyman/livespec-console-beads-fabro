...
Scenario: Each overridable setting has exactly one console command
  Given the orchestrator's overridable dispatcher settings, which at the time of writing are auto_approve_ready, acceptance_mode, merge_on_review_cap, review_fix_cap, and acceptance_rework_cap
  ...
  And every other overridable setting is served by `work_item.set_dispatcher_override_requested`
...
flowchart LR
  ...
  Move["narrowed per-lane targets: move"]
...
Scenario: The picker offers a pending-approval item only withdraw and park
  Given a selected `pending-approval` work-item in a drilled-in lane
  When the operator opens the move-to-status picker
  Then the offered targets are exactly `backlog` and `blocked`
  And `ready` is not offered because admission routes through the approve valve only

Scenario: The picker offers an acceptance item only de-scope and park
  Given a selected `acceptance` work-item in a drilled-in lane
  When the operator opens the move-to-status picker
  Then the offered targets are exactly `backlog` and `blocked`
  And `done` is not offered because shipping routes through the accept valve only
...
Scenario: Moving an item to a pre-terminal status routes through the guarded move action
  ...
  When the operator moves it to `backlog`, `ready`, or `blocked`
...
Scenario: No lane offers a move into active or done and the picker never un-ships a done item
  Given a selected work-item in any lane
  When the operator opens the move-to-status picker
  Then `active` is not offered from any lane because `active` is entered only by a journaled dispatch or a rework return from `acceptance`
  And `done` is not offered from any lane because `done` is reachable only via `accept` from `acceptance`
...
Scenario: A hint omits the per-item keys where no work-item is selected
  Given the Lanes view on the lane overview, whose selection is a lane and not a work-item
...
## Scenario 25 -- Per-item verb hints follow the per-state vocabulary

```mermaid
flowchart LR
  Select["Selected work-item"]
  Vocab["Per-state verb vocabulary (orchestrator-owned, consumed as data)"]
  Hints["Status-line hints"]
  Keys["Per-item keys"]

  Select --> Vocab
  Vocab -->|"state admits verb"| Hints
  Vocab -->|"state forbids verb"| Keys
```

```gherkin
Feature: Per-item verb suppression consumes the ratified per-state vocabulary
  As a LiveSpec operator
  I want per-item hints and keys to follow the selected item's lifecycle state
  So that every offered verb is actionable and a broken key is distinguishable from a mis-documented one

Scenario: A done item offers no verbs
  Given a selected `done` work-item in a drilled-in lane
  When the operator views the Status line
  Then no valve, move, or policy-dial hint is offered
  And the corresponding keys are inert

Scenario: A backlog item offers state-appropriate verbs only
  Given a selected `backlog` work-item in a drilled-in lane
  When the operator views the Status line
  Then approve and accept hints are absent
  And groom and move hints are present

Scenario: An acceptance item keeps its reject hint
  Given a selected `acceptance` work-item in a drilled-in lane
  When the operator views the Status line
  Then accept and reject hints are present
  And the suppression rule does not treat `active` as having a single entry door
```

## Scenario 26 -- Operator hands a work-item off to an LLM driver session

```mermaid
flowchart LR
  Backlog["backlog item: groom invocation"]
  Unsafe["factory-unsafe ready item: driver-implement invocation"]
  Overlay["Full-width overlay + optional OSC 52 (no claimed success)"]
  Operator["Operator pastes into a driver session"]
  Driver["Driver journals its own dispatch"]

  Backlog --> Overlay
  Unsafe --> Overlay
  Overlay --> Operator --> Driver
```

```gherkin
Feature: Driver handoff renders a copy-paste-safe invocation and executes nothing
  As a LiveSpec operator
  I want a lane-appropriate driver-handoff verb that renders the exact driver command
  So that heavyweight LLM-driven work reaches a driver session without the console depending on it

Scenario: A backlog item hands off to groom
  Given a selected `backlog` work-item
  When the operator invokes the driver handoff
  Then a full-width overlay renders the groom invocation carrying only the work-item id
  And the console spawns, monitors, and awaits nothing

Scenario: A factory-unsafe ready item hands off to driver-implement
  Given a selected `ready` work-item whose `factory_safety` is non-null
  When the operator invokes the driver handoff
  Then the overlay renders the driver-implement invocation carrying only the work-item id
  And the overlay never claims the copy succeeded

Scenario: A factory-safe ready item offers no driver handoff
  Given a selected `ready` work-item whose `factory_safety` is null
  When the operator views the per-item verbs
  Then the driver-handoff verb is suppressed
```

## Scenario 27 -- Every operator action is menu-reachable; hotkeys are only additional
...
flowchart LR
  ...
  Key["Hotkey (optional accelerator)"]
  Refusal["Refusal rendering"]

  Registry --> Menu --> Dialog
  Registry -.->|"may also bind"| Key --> Dialog
  Dialog -->|"orchestrator refuses"| Refusal
```

```gherkin
Feature: Menu-primary operation with hotkeys as additional accelerators
  As a LiveSpec operator
  I want every action reachable through menus and dialogs
  So that no action depends on memorizing a key and no refusal is silent

Scenario: A hotkey-free build keeps every action reachable
  Given the console is built with every hotkey binding disabled
  When the operator drives the session
  Then every registered action remains reachable through a menu path

Scenario: The generic traversal reaches every registered action
  Given the action registry
  When the generic menu-traversal test runs
  Then for every registered action there exists a menu path that reaches its dialog

Scenario: An unavailable action is presented unavailable and its hotkey is inert
  Given a registered action whose availability predicate is false for the selected item
  When the operator opens the menu
  Then that action is presented as unavailable
  And its hotkey is inert

Scenario: A refused action renders its refusal in the cockpit
  Given an action is invoked and the orchestrator refuses it
  When the refusal returns on the documented surface
...
```mermaid
flowchart LR
  Cmd["factory.dispatch_item_requested"]
  Port["Console per-item dispatch port"]
  Argv["loop --budget 1 --parallel 1 --item <id>"]
  Governed["Governed orchestrator surface"]
  Narrow["Narrows selection; never bypasses eligibility"]
  Gate["--item marks human hand-picked; cost gate keys on it"]

  Cmd --> Port --> Argv --> Governed
  Argv --> Narrow
  Argv --> Gate
```

```gherkin
Feature: Per-item dispatch is a hand-picked single-item run
  As a LiveSpec operator
  I want the per-item dispatch verb to bind the orchestrator's governed single-item surface
  So that dispatching one item narrows the ranked selection without bypassing eligibility or the cost gate

Scenario: The per-item launcher uses the governed bounded-loop surface
  Given a selected work-item in the ready lane
  When the operator requests a per-item dispatch and the console invokes the Dispatcher through its per-item port
  Then the invocation carries the loop subcommand bounded to one item and naming that work-item
  And the invocation carries no per-run policy-arming argument
  And the console does not invoke the ungoverned dispatch subcommand

Scenario: A named ineligible item is not dispatched
  Given a selected work-item that is not dispatch-eligible
  When the operator requests a per-item dispatch on it
  Then the item is not dispatched, exactly as if it had not been named
  And the console surfaces the no-dispatch outcome rather than reporting a dispatch

Scenario: An unconditional not-wired refusal is not a conformant per-item dispatch
  Given the specification names a concrete per-item launcher argv
  When the console's per-item port reports not-wired for every invocation
  Then the port does not conform to the per-item launcher contract
  And the verb is not presented to the operator as available
...
## Scenario 30 -- A needs-human terminal reaches the operator as a ledger valve

```mermaid
flowchart LR
  Run["Factory run ends at the needs_human terminal"]
  Ref["Tree preserved on refs/heads/needs-human/<run id>"]
  Ledger["Work-item rests at blocked / needs-human"]
  Projection["Orchestrator attention projection advertises resolve-blocked + rework"]
  Console["Console renders the valves; no attach command"]
  Run --> Ref
  Run --> Ledger --> Projection --> Console
```

```gherkin
Feature: Needs-human is a ledger valve, not a parked run
  As an operator
  I want a needs-human outcome to reach me as a work-item valve I can press
  So that no factory run ever waits on me and nothing asks me to attach to a run that is gone
...
Scenario: A needs-human terminal reaches the operator as a ledger valve
  ...
  And the orphaned-factory-runs lane lists a run only when the orchestrator's reconcile-runs projection reports it orphaned, with its orphan reason and termination route

Scenario: The Fabro adapter reads a run's status kind instead of assuming a human gate
  Given an observed factory run whose status kind is not a needs-human terminal
  When the Fabro adapter emits its events
  Then no human-gate state is synthesized for that run
  And the emitted terminal event carries the run's actual status kind

Scenario: The console's bundled workflow fork terminates at needs_human instead of parking
  Given a run of the console's bundled implement workflow reaches a needs-human outcome
  When the run ends
  Then it terminates at the needs_human node with its tree preserved on the needs-human ref
  And no in-loop escalate gate holds the run open awaiting a human
```
