...
**Inner loop -- `just check` (fast + deterministic; runs locally and,
except for a code-neutral documentation change classified below, in CI
on every push and pull request).** It MUST include:
...
- coverage gated at **100% line** and **100% region** coverage of
  ...
  **merged, cross-instantiation view** and both requiring ZERO
  ...
  That region target is defined
  against the merged reachable-region view rather than the raw
  `cargo llvm-cov --workspace --lib` **summary** region count, because
  that summary over-reports uncovered regions by an llvm-cov cross-object
  merge artifact: under `--workspace` a single non-generic library item
  is compiled into several crates' `--lib` test binaries and counted
  uncovered in the objects that link but never call it, even when every
  reachable region is exercised. The merged reachable-region view is the
  number reported by a scalar-max merge over `--json`
  `data[0].functions[].regions` (a region is covered iff its maximum
  execution count across instantiations is non-zero), equivalently by
  `llvm-cov show` across all workspace objects with
  `-show-instantiations=false` rendering no `^0`. Both bind **every**
  workspace library (`--lib`) target with **no per-crate carve-outs** --
  ...
  event store) is a redesign signal, not grounds for a carve-out. Reading
  the region target as the merged reachable-region view is a measurement
  correction, not a coverage exclusion: no region, arm, or line is
  exempted, and the **No coverage exclusions are permitted** clause above
  stands in full. The binary entry point (`main.rs`) is the only
  uncovered shim (e.g. `console-cli`'s `main.rs`); all testable logic
  lives in the covered library targets. (`--branch` coverage is unstable
  in `cargo llvm-cov`, so **line** and **region** coverage, both in the
  merged view, are the falsifiable knobs that gate today.)
...
**Merge gate -- CI on every pull request not classified as code-neutral
documentation-only (in addition to `just check`).** It MUST include:
...
**Code-neutral documentation lane -- CI on classified documentation-only
pull requests and canonical-branch pushes.** CI MUST classify a change as
code-neutral documentation-only only when every changed path is one of:

- `CHANGELOG.md`;
- a Markdown file beneath `.ai/` or `plan/`; or
- `docs/doc-custody.md` or `docs/factory-confirmations.md`.

Each changed endpoint MUST be a non-executable regular file (Git mode
`100644`) or the absent side of a regular-file addition or deletion; a type,
mode, symlink, or submodule change MUST select the full gate. Both source and
destination paths of a rename MUST satisfy the allow-list. `README.md`,
`AGENTS.md`, `CLAUDE.md`, and the documentation files consumed by Rust
lockstep and completeness tests are deliberately outside the allow-list. A
path beneath `SPECIFICATION/`, `.fabro/`, `.claude-plugin/`, `.github/`, a
source or test tree, or a manifest or lockfile MUST select the full gate even
when its filename ends in `.md`; every other unrecognized path MUST also
select the full gate. An empty diff, an unavailable base or head commit, or
any classifier error MUST fail closed to the full gate.

For a change classified as code-neutral documentation-only, CI MUST NOT
schedule the Rust compile/test matrix, the real-TUI end-to-end job, mutation
testing, or fuzzing. It MUST still run the change classifier and the exact
lightweight recipes `just check-doctor-static`, `just check-charters`, `just
check-plan-no-tombstone`, `just check-plan-record-conformance`, and `just
check-plan-anchor-declared`, and it MUST report the same required `ci-green`
context used by the full gate. The `ci-green` job MUST treat a skipped
code-only job as neutral only when the classifier succeeded and selected the
documentation lane. It MUST fail if any lightweight job is skipped, fails, or
is cancelled, or if any full-gate job is skipped after the classifier selected
the full gate. Every other change MUST run the full inner and merge gates
above. The documentation-only lane MUST NOT reduce the nightly gate.
...
the canonical branch; it MUST instead file a chore work-item in the
project's work-items ledger (the `livespec-console-beads-fabro`
tenant), through the tailnet beads write ingress described below. That
ingress files the finding with a single `bd create`; its request line
carries a fingerprint, a title, and an optional body, and no lifecycle
status, rank, or priority can be expressed through it -- `bd create`
has no status flag at all, and the ingress passes no priority. The
filed item therefore lands at the beads substrate's default `open`
status, OUTSIDE the orchestrator's work-item lifecycle states, and
unranked -- AT THE FILING STEP.
...
What the filing MUST guarantee is narrower and real: the finding is
durably recorded in the tenant, labelled `ci-soak-finding` and carrying
its fingerprint, so it is recoverable by label. A nightly finding MUST
NOT be filed into `ready` -- the orchestrator's adoption lands it at
`backlog`, never `ready`.
Nightly chore filing is **idempotent**: the nightly MUST derive a
stable finding signature -- for a fuzz crash, a hash of the reproducing
input (or the crash backtrace); for a surviving mutant, its `(source
file, line, mutation-operator)` identity -- and MUST render it as a
fingerprint matching `^[a-z0-9]([a-z0-9-]{0,62}[a-z0-9])?$` (a
DNS-label-like slug of at most 64 characters), which is the dedup key
and is persisted on the filed work-item. The ingress MUST suppress a
filing when it observes an open (non-closed) chore already carrying
that fingerprint, and file only when it observes none -- the suppression
is host-side, since the request surface is single-verb by design; once a
chore for a finding has been closed, a later recurrence of that finding
files again. The check is observational, not atomic: two concurrent runs
for the same finding MAY each observe no open chore and both file, and
this benign list-then-create race is ACCEPTED rather than closed, since
closing it would mean widening the single-verb surface that `dolt-server`
ratifies. The pre-existing open chore MAY be stale -- its detail can lag,
and it does not re-surface a new finding masked behind it -- which is
acceptable because the soak is expensive and any fix re-runs the soak
manually to surface the current finding set; the obligation is that each
distinct finding is tracked at least once, not that every open chore
stays current or that it is tracked exactly once.
That write ingress is an SSH forced command the shared Dolt host
exposes on the tailnet, accepting a single
`create <fingerprint> <base64url-title> [<base64url-body>]` request. It
performs the filing host-side under its own pinned `bd`, records the
fingerprint on the work-item, and takes a server-assigned id rather
than a caller-chosen deterministic one, so a re-file after a close
cannot collide with the closed chore's id.
CI MUST NOT hold the work-items database credential: it authenticates
to the ingress with an SSH key alone, and the database port remains
bound to loopback. The Beads/Fabro Family Secret Convention below
continues to govern this repository's own processes, not CI-held
credentials and not the ingress host's, which `dolt-server` governs
under its own sealed credential profile.
Because that ingress is reachable only from the tailnet, the nightly
MUST run on the self-hosted, on-tailnet runner pool; scheduled onto a
runner with no tailnet identity it MUST fail loudly rather than
complete while filing nothing.

```mermaid
flowchart TB
  Classify["classify changed paths (fail closed)"]
  subgraph Docs["code-neutral documentation lane"]
    Static["doctor-static + charter validation"]
    Tombstone["plan tombstone + record + anchor validation"]
  end
  subgraph Inner["just check -- full inner loop"]
    Fmt["fmt"]
    Clippy["clippy -D warnings"]
    Tests["nextest"]
    Coverage["coverage 100% line + region (lib, merged view)"]
  ...
  Classify -->|docs only| Docs --> Green["ci-green"]
  Classify -->|other / uncertain| Inner --> Merge --> Green
  Merge --> Night
  FuzzSoak --> Chore
  MutantsFull --> Chore
```
...
### Cross-Repo Action-Surface Parity

The console's action registry MUST be covered by a cross-repo PARITY
check asserting that the registry accounts for the orchestrator's
published human action surface, run inside `just check` and CI. An
action the orchestrator gains and the console never binds MUST be a red
build, never a silent gap.

Every deliberate omission from the parity mapping MUST carry a
recorded, non-empty reason -- the allowlist-with-reasons idiom -- so a
gap is either bound, or explained, never merely absent.

The check MUST be able to fail, demonstrated rather than assumed: it is
expected to be born red against any currently-unbound published valve
action, and that born-red run is its red demonstration. A parity check
that cannot be shown red is vacuous and MUST NOT satisfy this
requirement.
...
flowchart LR
  ...
  subgraph Inner["just check (full inner loop)"]
    ...
    Cov["100% line + region coverage, no carve-outs"]
    Props["property tests"]
  ...
  subgraph Night["nightly (canonical branch)"]
    ...
    Chore["finding -> chore filed unranked at beads open (adopted+ranked by orchestrator); never fail master"]
  end
  Classify -->|docs only| Docs --> Green["ci-green"]
  Classify -->|other / uncertain| Inner --> Merge --> Green
  Merge --> Night
...
Feature: Cost-and-determinism split of the contributor quality gate
  ...
  Scenario: The inner loop runs the fast deterministic checks
    ...
    Then it includes fmt --check, clippy denying warnings,
      cargo test and cargo nextest, 100% line and region coverage over every
  ...
  Scenario: The merge gate adds fuzzing and diff-scoped mutation
    Given a code-bearing pull request
  ...
  Scenario: A code-neutral documentation change uses the lightweight lane
    Given every changed path is CHANGELOG.md, a Markdown file beneath .ai or
      plan, or an explicitly allowed documentation file
    And neither side of a rename escapes that allow-list
    And every changed endpoint is a non-executable regular file or the absent
      side of a regular-file addition or deletion
    When CI classifies the pull request or canonical-branch push
    Then it does not schedule the Rust matrix, real-TUI end-to-end job,
      mutation testing, or fuzzing
    And it runs check-doctor-static, check-charters,
      check-plan-no-tombstone, check-plan-record-conformance, and
      check-plan-anchor-declared
    And it reports the same required ci-green context as the full gate
    And the skipped code-only jobs are neutral because the classifier selected
      the documentation lane

  Scenario: Classification uncertainty selects the full gate
    Given the changed paths are empty, unavailable, unrecognized, or include
      a behavior-bearing path, symlink, submodule, or mode change
    When CI classifies the change
    Then it runs the full inner and merge gates
    And a skipped full-gate job fails ci-green

  Scenario: The required aggregate fails closed in either lane
    Given the classifier selected either the documentation lane or the full gate
    When a required job for that lane is skipped, fails, or is cancelled
    Then ci-green fails
    And the same required context name is reported in both lanes
  ...
  Scenario: A nightly finding opens a chore instead of failing master
    ...
    And that filing carries the finding fingerprint and the
      ci-soak-finding label
...
flowchart LR
  ...
  Repo["this repository's own processes"]
  Ingress["dolt-server write ingress -- credentialed by dolt-server, not by this convention"]
  Beads["livespec-console-beads-fabro tenant"]
  CI["CI -- SSH key only, never holds the secret"]
  OP --> Bare --> Repo --> Beads
  CI -- "SSH key" --> Ingress --> Beads
```
...
## Contributor Scenario J -- Cross-repo action-surface parity

```mermaid
flowchart TB
  Upstream["Orchestrator published human action surface"]
  Registry["Console action registry"]
  Parity{"Parity check (just check + CI)"}
  Bound["Bound: registry action id"]
  Omitted["Omitted: recorded non-empty reason"]
  Red["Red build naming the action"]

  Upstream --> Parity
  Registry --> Parity
  Parity -->|"every action bound"| Bound
  Parity -->|"deliberate omission"| Omitted
  Parity -->|"unbound, no reason"| Red
```

```gherkin
Feature: Registry parity with the orchestrator's published action surface
  As a LiveSpec contributor
  I want an unbound upstream action to fail the build with a name
  So that the console's action set cannot silently fall behind the orchestrator

Scenario: An unbound published action is a red build
  Given the orchestrator publishes a human action the registry does not bind
  And no omission reason is recorded for it
  When the parity check runs
  Then the build fails naming that action

Scenario: A deliberate omission with a recorded reason passes
  Given a published action deliberately omitted from the registry
  And the omission carries a recorded non-empty reason
  When the parity check runs
  Then the check passes and the omission remains visible in the mapping

Scenario: The check demonstrates it can fail
  Given the parity check is introduced against a surface with unbound valve actions
  When it first runs
  Then it is born red naming those actions before the bindings or reasons land
```
