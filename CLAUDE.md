...
`refs/stash` is a single stack shared by every worktree of the repo — it is NOT
per-worktree, and nothing about `git stash push` in a worktree scopes it. When
two agents hold worktrees open at once, their pushes and pops interleave on that
one stack, and a `pop` can deliver another worktree's changes into yours while
your own entry is shuffled down or claimed by someone else.

Measured 2026-09-08, with two agents working concurrently: an agent implementing
`mx9u.11` wrapped a WIP edit in a scoped `git stash push -- <2 files>` /
`git stash pop`; a second agent was rebuilding the `mx9u.5` commits at the same
moment. The first agent's own stashed fix vanished from both the working tree and
the stash list, and `mx9u.5`'s entire uncommitted file set — `action_registry.rs`,
`console-application/src/lib.rs`, `console-cli/src/lib.rs`, `console-tui/src/lib.rs`,
`console-tui/tests/view_switch_keys.rs`, three `docs/` files and
`tests/fixtures/drive-human-action-surface.json` — landed in the `mx9u.11`
worktree instead. Nothing errored. Both agents' trees looked plausible.

- **Never run `git stash` in this repo while any other worktree is open.** To set
  work aside, commit it on a scratch branch in your own worktree, or simply leave
  it uncommitted — a worktree is already the isolation `stash` is not.
- **Never recover from the stash stack after a race.** Recover from commit
  objects, which are immutable and unambiguous: `git show --stat <sha>` to see
  what a commit held, `git reset --hard <sha>` to restore your own unmerged
  branch to it. A `pop` during a race can yank content out from under whichever
  agent is mid-operation.
- **A tree that is silently SHORT of content still commits.** After any
  suspected interference, diff what you have against the commit you expect
  (`git diff --stat $(git merge-base HEAD origin/master)` against
  `git show --stat <sha>`) before committing — a Green commit built from a
  half-swept tree passes the hook and looks like success.
